<template>
  <AuthenticatedLayout :navigation="navigation" role-title="Municipal Agriculturist" page-title="Messages">
    <template #header>
    ...
    <div class="flex flex-col md:flex-row gap-4 h-[calc(110vh-8rem)]">
      ...
      <div class="flex-1 flex flex-col bg-white border border-gray-300 rounded-2xl shadow-sm overflow-hidden">
        ...
        <template v-else>
          <!-- Header -->
          <div class="bg-white border-b border-gray-200 p-4 flex items-center gap-4 sticky top-0 z-10 shadow-sm">
            <button v-if="isMobile" class="p-2 rounded-full hover:bg-gray-100 transition" @click="backToList">
            ...
            <div class="relative">
              ...
              <span class="absolute bottom-0 right-0 block w-3 h-3 bg-green-400 border-2 border-white rounded-full"
                title="Online"></span>
            </div>
          ...
          <!-- Message List -->
          <div ref="messagesContainer" class="flex-1 overflow-y-auto p-5 space-y-4 bg-gray-100 scroll-smooth">
            ...
            <div v-for="message in messages" :key="message.id" :class="[
              ...
              <div class="max-w-[80%] md:max-w-xl space-y-2">
                ...
                <div v-if="message.attachments && message.attachments.length > 0" :class="[
                  ...
                  <div v-for="(attachment, index) in message.attachments" :key="attachment.documentId || index"
                    class="relative group">
                    <!-- Download Icon -->
                    <button @click="handleDownload(attachment.documentId)"
                    ...
                    <div v-if="isImageFile(attachment.url)" class="w-64 h-48">
                      <img :src="attachment.url" :alt="`Attachment ${index + 1}`"
                        ...
                        @click="openImageModal(attachment.url, message)" @error="handleImageError" />
                    </div>
                    ...
                    <p v-if="!message.text" class="text-xs text-gray-500 mt-1 px-1">
                      {{ formatTime(message.timestamp) }}
                    </p>
                  </div>
                </div>
          ...
          <div class="bg-white border-t border-gray-200 p-4">
            ...
            <div v-if="localFiles.length > 0" class="mb-3 p-3 bg-gray-50 rounded-lg border">
              <div class="flex items-center justify-between mb-2">
                ...
                <button type="button" @click="clearAllFiles" class="text-xs text-red-600 hover:text-red-800">
                  Clear all
                </button>
              </div>
              <div class="space-y-2 max-h-32 overflow-y-auto">
                <div v-for="(fileItem, index) in localFiles" :key="index"
                  class="flex items-center gap-3 p-2 bg-white rounded border">
                  <!-- File Preview -->
                  <div class="flex-shrink-0">
                    <img v-if="isImageFile(fileItem.file)" :src="getFilePreviewUrl(fileItem.file)" alt="Preview"
                      class="w-12 h-12 object-cover rounded border" />
                    <div v-else class="w-12 h-12 bg-gray-200 rounded flex items-center justify-center border">
            ...
            <form class="flex gap-3" @submit.prevent="sendChatMessage">
              <label
                ...
                <input ref="fileInput" type="file" multiple accept="image/*,.pdf,.doc,.docx,.txt" class="hidden"
      ...
      <!-- 🧑‍🌾 Farmer List Sidebar -->
      <div class="md:w-5/12 lg:w-96 flex flex-col bg-gray-100 border border-gray-300 rounded-2xl overflow-hidden">
        <!-- 🔍 Search Bar -->
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <div v-else>
            <button v-for="farmer in filteredFarmers" :key="farmer.id" :class="[
    ...
    <div v-if="showImageModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90"
      ...
      <div class="relative max-w-6xl max-h-full p-4 flex gap-4">
        ...
        <div class="flex-1 flex items-center justify-center">
          <img :src="selectedImage" alt="Full size image" class="max-w-full max-h-full rounded-lg" @click.stop />
        </div>

        <!-- Message Context Panel -->
        <div v-if="selectedMessageContext" class="w-80 bg-white rounded-lg p-4 overflow-y-auto max-h-full" @click.stop>
          <div class="flex items-center gap-3 mb-4 pb-3 border-b">
            <div class="w-10 h-10 rounded-full bg-green-600 text-white flex items-center justify-center font-medium">
          ...
          <div v-if="selectedMessageContext.attachments?.length > 1" class="space-y-2">
            ...
            <div v-for="(attachment, index) in selectedMessageContext.attachments" :key="attachment.documentId || index"
        ...
        <!-- Close Button -->
        <button class="absolute top-4 right-4 bg-white text-gray-800 rounded-full p-2 hover:bg-gray-100 z-10"
          @click="closeImageModal">
          ✕
        </button>
      </div>
