...
export const useWebSocketStore = defineStore('websocket', () => {
        ...
        privateMessageSub = stompClient.value.subscribe(topic, message => {
            ...
            try {
                ...
                if (
                    ...
                    data.attachmentResponses.length > 0
                ) {
                    console.log(
                        `[WebSocket] 📎 Message has ${data.attachmentResponses.length} attachment(s):`,
                        data.attachmentResponses.map(att => ({
    ...
    const subscribeToApplicationNotification = () => {
        const agriculture = '/topic/agriculture.application.notifications'
        const staff = '/topic/staff.application.notifications'
        if (!stompClient.value?.active) return
        if (notificationSub) notificationSub.unsubscribe()

        notificationSub = stompClient.value.subscribe(agriculture, message => {
            console.log('[WebSocket] 📨Agriculture Notification:', message.body)
            try {
                const data = JSON.parse(message.body)
        ...
        notificationSub = stompClient.value.subscribe(staff, message => {
            console.log('[WebSocket] 📨Staff Notification:', message.body)
            try {
                const data = JSON.parse(message.body)
                notificationStore.addIncomingNotifications(data)
            } catch (err) {
    ...
    const connect = async () => {
        ...
        if (stompClient.value?.active && connected.value)
    ...
    const scheduleReconnect = () => {
        ...
        const delay = 10_000
        console.log(
            `[WebSocket] ⏳ Reconnect attempt ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} in ${delay}ms`,
        )
