      ...
      <div class="flex-1 min-h-0 overflow-y-auto">
        ...
        <aside class="space-y-5">
          ...
          <div class="flex flex-col space-y-3 pt-4">
            <button
              @click="createAIClaim"
              :disabled="isCreatingClaim || !aiResult"
              class="w-full bg-green-600 text-white py-2 rounded-md text-sm font-semibold hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <span v-if="!isCreatingClaim">Create AI Claim</span>
...
// Create AI claim based on analysis results
const createAIClaim = async () => {
  if (!aiResult.value || !insuranceData.value) {
    toastStore.error('AI analysis or insurance data not available')
    return
  }
  ...
  try {
    ...
    // Prepare claim data based on AI analysis
    const claimData = {
      insuranceId: insuranceData.value.insuranceId,
      applicationId: submissionId,
      damageDescription: `AI detected damage: ${aiResult.value.analysis_results?.join(', ') || 'Crop damage detected'}`,
      aiAnalysisResults: aiResult.value,
      confidenceScore: aiResult.value.confidence || 0,
      detectedDiseases: aiResult.value.analysis_results || []
    }

    // Get supporting files from AI analysis images
    const supportingFiles = []
    
    // Note: In a real implementation, you might need to convert the AI analysis images
    // to File objects or handle them differently based on your backend requirements

    const result = await claimStore.createClaimFromAI(claimData, supportingFiles)

    if (result.success) {
      toastStore.success('AI claim created successfully!')
      
      // Navigate back to application detail or refresh data
      setTimeout(() => {
        navigateToApplicationDetail()
      }, 1500)
    } else {
      toastStore.error(result.message || 'Failed to create AI claim')
    }
  } catch (error) {
...
const fetchData = async () => {
  try {
    ...
    // Fetch insurance data to get application details
    const insuranceResponse = await insuranceStore.fetchInsuranceById(insuranceId)
    if (!insuranceResponse.success) {
      error.value = insuranceResponse.message || 'Failed to fetch insurance data'
      return
    }
    insuranceData.value = insuranceResponse.data

    // Fetch application data using submission ID
    const appResponse = await applicationStore.fetchApplicationById(submissionId)
    ...
    // Fetch AI result using submission ID
    const aiResponse = await aiStore.getAiResult(submissionId)
...
onMounted(() => {
  if (insuranceId && submissionId) {
    ...
    error.value = 'Insurance ID or Submission ID not found in route'
    loading.value = false
  }
